      ...
      <Center>
        <Content>
          ...
          <Text>
            ...
            minimal viewport width where this breakpoint will apply. By default the initial
...
configure({
  ...
  scale: {
    ...
    factor: 1,
          ...
          <Text>
            For even more fine grained control it's possible to override the default scaling
            function. This method called <InlineCode>value</InlineCode> will receive the number of
            the property. This would be 10 in the case of{' '}
            <InlineCode>{`{ padding: 10 }`}</InlineCode>. Additionally, the method will receive the
            current breakpoint. As shown below with this it's easily possible to calculate the value
            based on the breakpoint instead of scaling linearly.
          </Text>
          <Code
            backgroundColor={theme.color.codeBackground}
          >{`import { configure } from 'responsive-react-native'
