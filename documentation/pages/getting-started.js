    ...
    <>
      ...
      <ContentGrid>
        ...
        <Text>
          ...
          Stylesheets used in React Native and automativally makes any scaleable values responsive.
          This approach requires that the whole application rerenders when the viewport changes.
          Usually, this is not an issue since viewport changes on mobile devices are rare. The
          second way uses styled components and requires a separate definition for each component.
          Using the latter interface requires more effort and is only recommended when starting with
          a new project. This interface doesn't require any rerenders and can conditionally apply
          styles based on component props.The <TextLink href="/styled">Styled API</TextLink>{' '}
          approach is explained in detail on a dedicated page.
        </Text>
        <Text>
  ...
  <View style={styles.view}>
    <Text style={styles.text}>Hello Responsive World</Text>
        ...
        <Text>
          ...
          It's possible to <TextLink href="/configuration">configure</TextLink> the default viewport
          as well as the strength of the scaling.
        </Text>
        <Title>Rerendering</Title>
        <Text>
          Since Stylesheets are statically initialized and frozen at the top of the file they cannot
          be made to automatically adapt during rendering. The <InlineCode>createStyles</InlineCode>{' '}
          method works around this by returning a proxied object that can adapt based on the
          viewport. However, a full rerender of the application is still necessary when the viewport
          changes in order to get the new styles. Any part of the application wrapped in{' '}
          <InlineCode>{`<Rerender>`}</InlineCode> automatically rerenders on changes to the
          viewport. Usually this component is added directly at the application root as in the
          example below.
        </Text>
        <Code
          backgroundColor={theme.color.codeBackground}
...
Object.assign(StyleSheet, { create: createStyles })`}</Code>
        <Text>
          ...
          occurs right away when the file is imported. This means that the override needs to happen
          before any components are imported. Placing the above code in a separate file and
