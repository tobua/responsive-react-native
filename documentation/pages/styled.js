...
export default () => <CustomView highlight />`}</Code>
          <Text>
            ...
            element or the element itself. The <Important>second argument</Important> takes the
            styles just like <InlineCode>createStyles</InlineCode> does. Scaleable values will
            automatically adapt linearly based on the viewport and adaptive values can be used to
            specify values for specific breakpoints or orientations. An optional{' '}
            <Important>third argument</Important> takes an object which can contain styles which
            will dynamically be applied if a truthy prop, a breakpoint, the operating system or the
