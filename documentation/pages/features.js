...
import { Center, Content, Title, Important, Text, InlineCode } from '../markup/General'
      ...
      <Center>
        <Content>
          ...
          <Text>
            ...
            and static values are still preferred. Luckily, it's very easy to use and will work out
            of the box. All values influencing the scaling can also be{' '}
          ...
          <Text>
            ...
            scaling <InlineCode>factor</InlineCode> which is <InlineCode>0.5</InlineCode> by
            default. A factor of one would cut the value in half in this case while the default of{' '}
          ...
          <Text>
            ...
            into existing applications. Due to the heavy use of pixel based scaling on mobile
            devices there is less fragmentation when it comes to the range of target viewports. This
            is one of the major reasons why responsive design currently isn't very popular for
            mobile devices. Still, there is slight fragmentation which is very well adressed by
            using scaled values which make a design created on a default device look the same on any
            kind of device. The issue of too much whitespace on large devices as well as an
            overcrowded design on smaller devices is gone.
          </Text>
          ...
          <Text>
            ...
            breakpoint or orientation. The syntax is deliberately kept terse to make responsive
            styles quick to write. <Important>Breakpoints</Important> are written as objects with
            their specific breakpoint as the key. If the current breakpoint is missing the nearest
            one below will be used. The values will no longer scale and are the same in any viewport
            where the breakpoint applies.
          </Text>
          <Code
...
const styles = createStyles({
  view: {
    height: { small: 40, large: 80 }
          ...
          <Text>
            Values that should match the orientation are written as arrays where the first key
            applies for portrait and the second for landscape.
          </Text>
          <Code
            backgroundColor={theme.color.codeBackground}
          >{`import { createStyles } from 'responsive-react-native'
  ...
  view: {
    backgroundColor: ['blue', 'red']
          ...
          <Text>
            ...
            any time the breakpoint or orientation changes.
          </Text>
          <Code
            backgroundColor={theme.color.codeBackground}
